...
public interface ExternalDataStructAPI {
    @PostMapping(value = APP_ROOT+"/DonneesPartenaire/saveAll/{sheetAt}/{referencePositionAt}/{montantPosition}",consumes = MediaType.MULTIPART_FORM_DATA_VALUE,produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    String saveAll(@RequestPart("file") MultipartFile file,@PathVariable("sheetAt") Integer sheetAt,@PathVariable("referencePositionAt") Integer referencePositionAt,@PathVariable("montantPosition") Integer montantPosition);
